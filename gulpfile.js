...
gulp.task('less', function () {
    return gulp.src('assets/less/bep.less')
...
gulp.task('build-static', ['clean-static', 'less', 'scripts', 'copy'], function () {
    return gulp.src(['static/assets/js/bep.js', 'static/assets/css/bep.css', 'layouts/partials/head.html'], {base: path.join(process.cwd(), 'static')})
        .pipe(plugins.revAll({
    ...
    return plugins.gitinfo()
        .pipe(es.map(function(data, cb) {
            fs.writeFile('layouts/partials/gitinfo.html',
                    'Den aller siste endringa var <a href="https://github.com/bjornerik/beps.is/commit/' + data['local.branch.current.shortSHA'] + '">'
...
gulp.task('aws-publish', ['build'], function () {
    ...
    var publisher = plugins.awspublish.create(JSON.parse(fs.readFileSync(process.env.HOME + '/.aws/bep.is.json')));
